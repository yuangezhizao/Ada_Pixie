...
#if defined CONFIG_UPLOAD_FORMAT_G711A || defined CONFIG_UPLOAD_FORMAT_G711U || defined CONFIG_UPLOAD_FORMAT_PCM
...
esp_err_t audio_manager_init(void)
{
    ...
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { ESP_LOGW(TAG, "Failed to set playback volume, continuing..."); }, "Failed to set playback volume");

    ret = esp_codec_dev_set_in_gain(audio_manager.rec_dev, DEFAULT_RECORD_DB);
    ESP_GMF_RET_ON_NOT_OK(
        TAG, ret, { ESP_LOGW(TAG, "Failed to set record volume, continuing..."); }, "Failed to set record volume");
...
{
    ESP_LOGD(TAG, "CB: RECV Pipeline EVT: el:%s-%p, type:%d, sub:%s, payload:%p, size:%d,%p", OBJ_GET_TAG(event->from),
             event->from, event->type, esp_gmf_event_get_state_str(event->sub), event->payload, event->payload_size,
             ctx);
    return 0;
}
...
static void esp_gmf_afe_event_cb(esp_gmf_obj_handle_t obj, esp_gmf_afe_evt_t* event, void* user_data)
{
    ...
    switch (event->type) {
        ...
        default: {
            ...
            ESP_LOGW(TAG, "Command %d, phrase_id %d, prob %f, str: %s", event->type, info->phrase_id, info->prob,
                     info->str);
            break;
        }
    }
...
static esp_err_t recorder_setup_afe_config(void)
{
    ...
    esp_gmf_afe_manager_cfg_t afe_manager_cfg =
        DEFAULT_GMF_AFE_MANAGER_CFG(audio_recorder.afe_cfg, NULL, NULL, NULL, NULL);
...
static void recorder_get_pipeline_config(const char*** elements,
                                         int* element_count,
                                         char** input_name,
                                         char** output_name)
{
...
static esp_err_t recorder_setup_pipeline_ports(const char* input_name, const char* output_name)
{
    esp_gmf_port_handle_t outport = NEW_ESP_GMF_PORT_OUT_BYTE(
        recorder_outport_acquire_write, recorder_outport_release_write, NULL, NULL, 2048, portMAX_DELAY);
    ...
    esp_gmf_port_handle_t inport = NEW_ESP_GMF_PORT_IN_BYTE(recorder_inport_acquire_read, recorder_inport_release_read,
...
esp_err_t audio_recorder_read_data(uint8_t* data, int data_size)
{
    ...
    if (bytes_to_copy > data_size) {
        ...
        ESP_LOGW(TAG, "Read data size is larger than expected, bytes_to_copy: %d, data_size: %d", bytes_to_copy,
                 data_size);
    }
...
static int playback_outport_acquire_write(void* handle,
...
static int playback_inport_acquire_read(void* handle, esp_gmf_data_bus_block_t* blk, int wanted_size, int block_ticks)
{
    ...
    int ret = esp_gmf_db_acquire_read(audio_playback.fifo, &_blk, wanted_size, block_ticks);
    if (ret < 0) {
...
{
    esp_gmf_port_handle_t inport = NEW_ESP_GMF_PORT_IN_BYTE(playback_inport_acquire_read, playback_inport_release_read,
                                                            NULL, NULL, 2048, portMAX_DELAY);
    ...
    esp_gmf_port_handle_t outport = NEW_ESP_GMF_PORT_OUT_BYTE(
        playback_outport_acquire_write, playback_outport_release_write, NULL, NULL, 2048, portMAX_DELAY);
...
esp_err_t audio_playback_open(void)
{
    ...
    err = esp_gmf_pool_new_pipeline(audio_manager.pool, NULL, elements, sizeof(elements) / sizeof(char*), NULL,
                                    &audio_playback.pipe);

...
esp_err_t coze_chat_app_init(void)
{
    ...
    button_adc_config_t btn_adc_cfg = {
        .unit_id = ADC_UNIT_1, .adc_channel = 4, .button_index = 0, .min = 2310, .max = 2510};
    ...
    esp_gmf_oal_thread_create(&coze_chat.read_thread, "audio_data_read_task", audio_data_read_task, (void*)NULL, 4096,
                              12, true, 1);

...
static void example_ledc_set_lcd_brightness(uint32_t percentage)
{
    ...
    if (percentage > 100) {
        ESP_LOGE(TAG_LCD, "BL brightness percentage %" PRIu32 "%% is out of range (0-100), resetting to default 50%%",
                 percentage);
        percentage = 50;
    }
...
esp_err_t app_lcd_init(void)
{
    ...
    const spi_bus_config_t buscfg = {
        ...
        .max_transfer_sz = EXAMPLE_LCD_H_RES * EXAMPLE_LCD_DRAW_BUFF_HEIGHT *
                           sizeof(uint16_t), // Note: EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * sizeof(uint16_t)};
    };
